...
const ProceedBuy = (props) => {
	...
	const { price, cryptoSymbol, payWith = '', amount, status, loading, onConfirm } = props
			...
			{!status && (
				<>
					<div className="text-center my-8">
						...
						<h3 className="font-bold">{price} {cryptoSymbol}</h3>
					</div>
					<InfoItem
						label="Pay with"
						value={
							(() => {
								const payment = paymentList.find(p => p.value === payWith)
								return (
									<div className="flex items-center gap-1">
										<img className="max-w-[35px]" src={payment.img} alt="" />
