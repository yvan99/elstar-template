...
const articleTagClass = (category) => {
	switch (category) {
		case 'themes':
		...
		<>
			{data.map(article => (
				<article key={article.id}>
					<Card className="group mb-4" clickable onClick={() => onArticleClick(article.id)}>
						<div className="px-8 py-3 relative">
							...
							<div className="flex items-center justify-between mb-2">
								...
								<Tag className={`border-0 rounded capitalize ${articleTagClass(article.category)}`}>
									{article.category}
								</Tag>
							</div>
