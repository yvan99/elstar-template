...
const MockApi = () => {
	return (
		<>
			<p>A mock API is an API that responds data just like a real API to do, however it is just imitating the data & response. This is extremely useful in many cases, like when the API you rely on isn't fully developed yet, or simulate API calls on unit testing.</p>
			...
			<div className="mt-10" id="disableMockApi">
				...
				<p className="mt-1">
					Mock api was enabled by default in our <strong>starter-kit</strong>, you can turn in on via setting <code>enableMock</code> field to false in <code>src/configs/app.config.js</code> to turn off the mock api.
				</p>
